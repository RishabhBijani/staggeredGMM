#' @title GMM I: Staggered Treatment Effect Estimator (pooled Toeplitz weighting)
#'
...
#'   using only clean 2x2 DiD comparisons as moment conditions. Every cohort
#'   shares a single pooled Toeplitz (stationary, homoskedastic)
#'   autocovariance sequence. See [gmm_staggered_II()] (cohort-specific
#'   Toeplitz) and [gmm_staggered_III()] (fully unrestricted within-cohort
#'   covariance) for the other two weightings.
#'
...
#' @param id_var Character. Name of the column in `data` that uniquely
#'   identifies each cross-sectional unit (e.g. individual, firm, county)
#'   tracked over time.
#' @param gname Character. Name of the cohort (treatment-timing group)
#'   column in `data`: each unit's first treated period, with `0` reserved
#'   for units that are never treated.
#' @param has_nt Logical, default `FALSE`. If `TRUE`, units with `gname == 0`
#'   are included as a never-treated control group, in addition to (not
#'   instead of) not-yet-treated cohorts, which always serve as controls
#'   whenever available regardless of `has_nt`. Setting this to `TRUE` when a
#'   never-treated group exists typically improves precision and identifies
#'   more CATTs.
#' @param max_iter Integer, default `10L`. Maximum number of iterated-GMM
#'   reweighting steps: the moment-covariance model is re-estimated from the
#'   current CATT estimates and the weighting is updated, repeating until the
#'   estimates stop changing appreciably or this cap is reached.
#' @param covar Optional character vector of baseline (pre-treatment,
#'   time-invariant) covariate column names in `data`, e.g. `c("x1", "x2")`.
#'   Default `NULL` (no covariate adjustment; `""` or `character(0)` are
#'   treated identically to `NULL`). When supplied, applies the
#'   outcome-regression adjustment of Section 4.5 of the paper, which allows
#'   parallel trends to hold only after conditioning on `covar` rather than
#'   unconditionally.
#'
#' @return A list with components:
#'   \describe{
#'     \item{beta_hat}{Numeric vector of estimated CATTs, one per (cohort,
#'       post-period) cell, in the order given by `cells`.}
#'     \item{SE_catt}{Numeric vector of standard errors, aligned to `beta_hat`.}
#'     \item{catt_out}{Data frame with columns `g`, `t`, `beta_hat`, `SE`, one
#'       row per estimated CATT.}
#'     \item{ATT_CW}{Aggregate ATT, weighted by each cohort's share of treated
#'       observations.}
#'     \item{SE_CW}{Standard error of `ATT_CW`.}
#'     \item{w_CW}{The aggregation weight vector underlying `ATT_CW`.}
#'     \item{cells}{Data frame of (cohort, post-period) cells, in `beta_hat`'s
#'       order.}
#'     \item{eff_ok}{Logical. `TRUE` if the efficient GMM reweighting converged
#'       without a matrix-solve failure; `FALSE` if estimation fell back to
#'       the identity-weighted seed with a sandwich standard error.}
#'     \item{n_iter}{Integer number of iterations performed.}
#'     \item{N_beta}{Integer number of estimated CATTs.}
#'     \item{N_2x2}{Integer number of clean 2x2 comparisons underlying the
#'       moment system.}
#'     \item{Q_H_rank}{Integer column rank of the incidence matrix Q_H; less
#'       than `N_beta` indicates at least one CATT is not identified by clean
#'       comparisons under this design (see Warnings).}
#'   }
#'
...
gmm_staggered_I <- function(data, y_var, t_var, id_var, gname,
                             has_nt   = FALSE,
                             max_iter = 10L,
                             covar    = NULL) {
  ...
  for (iter in seq_len(max_iter)) {
    ...
    for (d in 0L:(TT - 1L)) {
      r1 <- seq_len(TT - d)
      r2 <- (1L + d):TT
      sigma_d[d + 1L] <- sum(R_mat[, r1] * R_mat[, r2], na.rm = TRUE) /
        (N_units * (TT - d))
    }
    ...
    for (gi in seq_len(n_grp)) {
      ...
      W[cols, cols] <- toeplitz(sigma_d) / N_g
    }
    W_out <- W
