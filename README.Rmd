...
`"unrestricted"` needs the panel to support it. Each cohort's covariance is
...
weighted step completes and the estimator warns that it has fallen back to
the identity-weighted seed.
...
`inst/scripts/get_cook_shah_nregs.R` retrieves and prepares it on request.
